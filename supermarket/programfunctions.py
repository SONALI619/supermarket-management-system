...
def creating_new_user(a, b, c, d, e):
    nam = a+' '+b
    phone = c
    mail = d
    pswd = e
...
def update_food_db(a, b, c, d, k):
    try:
        ...
        q4= f"UPDATE food_base SET quantity='{d}' WHERE Food='{k}'"
        cr.execute(q1)
        cr.execute(q2)
        cr.execute(q3)
        cr.execute(q4)
...
def add_food_in_db(v):
    try:
        conn = sqlite3.connect('food.db')
        cr = conn.cursor()
        q = f"INSERT INTO food_base(Food, Rate, Place ,quantity) VALUES('{v[0]}', {v[1]}, '{v[2]}','{v[3]}')"
        cr.execute(q)
        conn.commit()
        conn.close()
        return True
